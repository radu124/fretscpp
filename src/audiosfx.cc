...
/**
 * Initializes all pointers to NULL and all lengths to 0 on instance creation
 */
tEffectList::tEffectList():count(0)
{
	int i;
	for (i=0; i<256; i++)
	{
...
int sfx_playbackready()
{
	...
	for (i=0; i<EFFECTSPLAY; i++)
	{
		...
		VDBG(SFX,"%d(%d/%d V:%d:%d) ", eid, playingeffects[i].pos, effectdata.len[eid],
